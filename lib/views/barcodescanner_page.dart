...
class _BarcodeScannerPageState extends State<BarcodeScannerPage> {
  ...
  Future<void> _initializeCamera() async {
    try {
      ...
      setState(() {
        errorMessage = 'Erreur d\'initialisation: ${e.toString()}';
      });
    }
  }
  ...
  Future<void> _switchCamera() async {
    ...
    try {
      ...
      // Flash state might reset when switching cameras
      setState(() {
        isFlashOn = false;
      });
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Erreur changement caméra: ${e.toString()}'),
          backgroundColor: Colors.red,
        ),
      );
    }
    ...
    return SafeArea(
      child: Scaffold(
        ...
        appBar: AppBar(
          ...
          actions: [
            // Flash toggle button with state indication
            IconButton(
              icon: Icon(
                isFlashOn ? Icons.flash_on : Icons.flash_off,
                color: isFlashOn ? Colors.yellow : Colors.white,
              ),
              onPressed: cameraController != null ? _toggleFlash : null,
              tooltip: isFlashOn ? 'Désactiver le flash' : 'Activer le flash',
            ),
            // Camera switch button
            IconButton(
              icon: const Icon(Icons.flip_camera_ios, color: Colors.white),
              onPressed: cameraController != null ? _switchCamera : null,
        ...
        body: Stack(
          children: [
            ...
            // Scanner overlay
            if (errorMessage == null && cameraController != null)
              Container(
