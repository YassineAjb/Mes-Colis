...
class RunsheetViewModel extends ChangeNotifier {
  ...
  List<Runsheet> get runsheets => _runsheets;
  ...
  Future<void> fetchRunsheets({bool refresh = false}) async {
    ...
    try {
      ...
      if (result['status'] == 'success') {
        ...
        if (refresh) {
          _runsheets = newRunsheets;
        } else {
          _runsheets.addAll(newRunsheets);
        }
        
        _currentPage++;
        _hasMoreData = newRunsheets.length == 10;
      } else {
        _errorMessage = result['message'];
      }
    } catch (e) {
  ...
  void setFilters({int? carId, String? fromDate, String? toDate}) {
